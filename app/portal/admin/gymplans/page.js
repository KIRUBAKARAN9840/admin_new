...
export default function GymPlans() {
  ...
  const { role } = useRole();
  const planType = searchParams.get("type") || "session"; // session, membership, or dailyPass
  ...
  const [priceSort, setPriceSort] = useState(searchParams.get("price_sort") || "");
  ...
  }, [searchTerm, sortOrder, priceSort, currentPage, itemsPerPage, planType, searchParams]);
  ...
  const fetchGyms = useCallback(async () => {
    try {
      ...
      // Apply plan type filter based on URL parameter
      if (planType === "session") {
        params.has_session_plans = true;
      } else if (planType === "membership") {
        params.has_membership_plans = true;
      } else if (planType === "dailyPass") {
        params.has_daily_pass = true;
      }

      // Apply price sort for session, membership, and daily pass plans
      if ((planType === "session" || planType === "membership" || planType === "dailyPass") && priceSort) {
  ...
  }, [debouncedSearchTerm, sortOrder, currentPage, itemsPerPage, planType, priceSort]);
    ...
    <div className="users-container">
      <div className="users-header">
        <div style={{ display: "flex", alignItems: "center", gap: "1rem" }}>
          ...
          <h2 className="users-title" style={{ margin: 0 }}>
            <span style={{ color: "#FF5757" }}>Gym</span> Plans - {getPlanTypeTitle()}
          </h2>
        </div>
      ...
      <div className="filters-section">
        <div className="row pb-0">
          ...
          {(planType === "session" || planType === "membership" || planType === "dailyPass") && (
            <div className="col-lg-2 col-md-6 col-sm-12">
              <select
