...
export default function BDMTracker() {
  ...
  const [hasSelfAssignedConverted, setHasSelfAssignedConverted] = useState(false);
  ...
  const fetchTrackerData = async () => {
    try {
      ...
      const summaryResponse = await axiosInstance.get(
        `/marketing/gym-visits/tracker/${bdmId}/BDM`
      );

      if (summaryResponse.data.status === 200) {
        setTrackerSummary(summaryResponse.data.summary);
        setBdmName(summaryResponse.data.user_profile?.name || "Manager");
      }

      // Fetch all tab counts in parallel
      const [pendingRes, followupRes, convertedRes, rejectedRes] = await Promise.all([
        axiosInstance.get(`/marketing/gym-visits/tracker/pending?user_id=${bdmId}&role=BDM`),
        axiosInstance.get(`/marketing/gym-visits/tracker/${bdmId}/BDM/followups`),
        axiosInstance.get(`/marketing/gym-visits/tracker/${bdmId}/BDM/converted`),
        axiosInstance.get(`/marketing/gym-visits/tracker/${bdmId}/BDM/rejected`)
      ]);
      ...
      // Check if there are any self-assigned converted visits
      const hasSelfAssigned = Array.isArray(convertedData) && convertedData.some(visit => visit.is_self_assigned);
      setHasSelfAssignedConverted(hasSelfAssigned);

      // If no self-assigned converted visits, don't proceed with setting data
      if (!hasSelfAssigned) {
        setLoading(false);
        return;
      }
      ...
      // Store data only for the active tab
  ...
  const fetchTabData = async (tab) => {
    ...
    try {
      ...
      switch (tab) {
        case "pending":
          response = await axiosInstance.get(`/marketing/gym-visits/tracker/pending?user_id=${bdmId}&role=BDM`);
          ...
          break;
        case "followup":
          response = await axiosInstance.get(`/marketing/gym-visits/tracker/${bdmId}/BDM/followups`);
          if (response.data.status === 200) {
            const visits = response.data.data || response.data;
          ...
          break;
        case "converted":
          response = await axiosInstance.get(`/marketing/gym-visits/tracker/${bdmId}/BDM/converted`);
          if (response.data.status === 200) {
            const visits = response.data.data || response.data;
          ...
          break;
        case "rejected":
          response = await axiosInstance.get(`/marketing/gym-visits/tracker/${bdmId}/BDM/rejected`);
          if (response.data.status === 200) {
            const visits = response.data.data || response.data;
  ...
  // If no self-assigned converted visits, render empty page
  if (!loading && !hasSelfAssignedConverted) {
    return (
      <div className="users-container">
        <div className="users-header">
          <div style={{ display: "flex", alignItems: "center", gap: "1rem" }}>
            <button
              onClick={() => router.push("/portal/admin/marketing/bdms")}
              style={{
                display: "flex",
                alignItems: "center",
                padding: "0",
            ...
            <h2 className="users-title">
              <span style={{ color: "#FF5757" }}>{bdmName}</span> Report
            </h2>
          </div>
        </div>
        <div style={{
          ...
          <p style={{ fontSize: "16px", margin: 0 }}>No self-assigned converted visits found</p>
        </div>
      </div>
    );
  }
