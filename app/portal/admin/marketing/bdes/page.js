...
export default function BDEs() {
  const router = useRouter();
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState(null);
  const [bdesData, setBdesData] = useState([]);
  ...
  const fetchBDEsList = async () => {
    try {
      ...
      if (response.data.status === 200) {
        setBdesData(response.data.data);
      } else {
  ...
  const filteredAndSortedData = useMemo(() => {
    let filtered = bdesData.filter((item) => {
      const searchMatch =
    ...
    return filtered;
  }, [bdesData, searchTerm, sortField, sortOrder]);
      ...
      <div className="users-container">
        <div className="users-header">
          <h2 className="users-title">
            <span style={{ color: "#FF5757" }}>BDEs</span> Performance
          </h2>
        </div>
    ...
    <div className="users-container" style={{ position: "relative" }}>
      ...
      <div className="users-header">
        ...
        <span className="users-count">
          Total BDEs: {filteredAndSortedData.length}
        </span>
      </div>
            ...
            <table className="users-table">
              <thead>
                <tr>
                  ...
                  {/* <th onClick={() => handleSort("conversion_ratio")} className="sortable">
                    Conversion Ratio (%)
              ...
              <tbody>
                {paginatedData.length > 0 ? (
                  paginatedData.map((bde, index) => (
                    <tr
                      key={bde.id}
                      onClick={() =>
                        router.push(`/portal/admin/marketing/bdes/${bde.id}`)
                      }
                      style={{ cursor: "pointer" }}
                    >
                      <td>
                        <div className="user-name">{bde.rank}</div>
                      </td>
                      <td>
                        <div className="user-name">{bde.name}</div>
                        <div style={{ fontSize: "12px", color: "#888" }}>
                          {bde.email}
                        </div>
                      </td>
                      <td>{bde.total_assigned}</td>
                      <td>{bde.total_visited}</td>
                      <td>{bde.total_converted}</td>
                      {/* <td>
                        <span
                          style={{
                            fontWeight: "600",
                            color: bde.conversion_ratio > 50 ? "#10b981" : bde.conversion_ratio > 25 ? "#f59e0b" : "#ef4444"
                          }}
                        >
                          {bde.conversion_ratio}%
                        </span>
                      </td> */}
                    </tr>
                  ))
                  ...
                  <tr>
                    <td colSpan="6" className="no-data">
                      No BDEs found matching your criteria
                    </td>
                  </tr>
                )}
              </tbody>
