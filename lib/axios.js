...
const refreshToken = async () => {
  try {
    ...
    if (typeof window !== "undefined") {
      console.warn("[AXIOS] Redirecting to login due to token refresh failure");
...
const verifyToken = async () => {
  try {
    ...
    console.error("[AXIOS] Token verification error:", error);
    ...
    if (error.response?.status === 401) {
      try {
        ...
        console.error(
          "[AXIOS] Token refresh failed during verification",
          refreshError
        );
      }
    }
...
axiosInstance.interceptors.response.use(
  ...
  async (error) => {
    ...
    if (!originalRequest || originalRequest._retry) {
      console.warn("Request already retried, aborting");
      return Promise.reject(error);
    }
    ...
    try {
      ...
      console.warn("Token refresh unsuccessful, redirecting to login");
      if (typeof window !== "undefined") {
        localStorage.removeItem("user");
        window.location.href = "/";
      }
